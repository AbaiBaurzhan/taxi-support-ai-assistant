#!/usr/bin/env python3
"""
🚀 Senior AI Engineer - Интеграция профессиональной системы в основное приложение
Максимальное качество поиска с профессиональной системой
"""
...
class SeniorAIIntegratedClient:
    ...
    def get_enhanced_answer(self, question: str) -> str:
        """
        Основной метод для получения ответа - интегрированный с профессиональной системой
        """
        start_time = datetime.now()
        ...
        results = self.hybrid_search_advanced(question, top_k=3)
...
senior_ai_integrated_client = SeniorAIIntegratedClient()

def get_enhanced_answer(question: str) -> str:
    """Основной API для интеграции с main.py"""
    return senior_ai_integrated_client.get_enhanced_answer(question)

def get_senior_ai_statistics() -> Dict[str, Any]:
    """API для получения статистики Senior AI системы"""
    return senior_ai_integrated_client.get_statistics()

if __name__ == "__main__":
    # Тестируем интегрированную систему
    client = SeniorAIIntegratedClient()
    
    # Показываем статистику
    stats = client.get_statistics()
    print(f"📊 Статистика интегрированной Senior AI системы:")
    ...
    for category, count in final_stats.get('category_distribution', {}).items():
